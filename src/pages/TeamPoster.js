...
class TeamPoster extends React.Component {
  ...
  async loadTeam(teamId, eventId, automate = false) {
    ...
    console.log(`teamData ${teamId}/${eventId}`, teamData);
  ...
  async fetchSportConfig(sport) {
    // API UPDATE
    const response = await fetch(`${this.API_BASE_URL}/product/team/${sport}`);
  ...
  onSubmitHandler = async () => {
    const formData = new FormData();
    const { sport, isAuto, eventId, teamId, isBanner, vendorName } = this.state;
    const params = [
      "isOnePhotoMode",
      "rightPhotoOffset",
      "headerCopy",
    ...
    axios
      .post(`${this.API_BASE_URL}/product/${type}/${sport}`, formData, config)
      .then(async ({ data }) => {
        if (data.statusCode !== 200) {
          throw new Error(`${data.error.type} : ${data.error.description}`);
        }
          ...
          <div className="app-control-container">
            ...
            <label className="app-control">
              ...
              >
                ...
                <option value="usa_softball">USA softball</option>
              </select>
            </label>
